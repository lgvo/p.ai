...
func (s *Store) ActiveWorkspaceInspect(ctx context.Context, sessionUUID string) (Operation, bool, error) {
	...
	var id string
	err := s.db.QueryRowContext(ctx, `SELECT id FROM operations WHERE session_uuid=? AND kind IN ('workspace.inspect','workspace.loss.inspect','session.discard','session.delete','session.rename','session.repair','session.repair.prepare','session.ref.repair','session.principal.repair')
